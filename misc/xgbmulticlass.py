def main(indices, ARGS):
    for index in indices:
        ...
        df=pd.read_csv("/cluster/scratch/dominikz/raw_data/trainset_full.csv", low_memory=False)
        ...
        #parameters={"n_estimators":[80,90,100,120], "learning_rate":[0.1,0.2], "max_depth":[4,6,7,8], "gamma":[0,0.1],"alpha":[0,0.02,0.1]}
        parameters={"n_estimators":[100], "learning_rate":[0.1], "max_depth":[6], "gamma":[0.1]}

        xgb_cv=GridSearchCV(xgb, parameters, scoring="f1_macro",cv=10, n_jobs=-1)
        xgb_cv.fit(X_train_xgb,y_train_xgb)

        print("Best parameters:", xgb_cv.best_params_ , ", Best CV Accuracy:", xgb_cv.best_score_)
        with open('/cluster/scratch/dominikz/final_multiclass.pkl', 'wb+') as f:
