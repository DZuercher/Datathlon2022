def main(indices, ARGS):
    ...
    class SIB:
        ...
        def train_xgb(self, df):
            ...
            for at in self.missing_attributes:
                ...
                # params={"n_estimators":[80,90,100,120], "learning_rate":[0.1,0.2], "max_depth":[4,6,7,8], "gamma":[0,0.1],"alpha":[0,0.02,0.1]}
                params={"n_estimators":[100], "learning_rate":[0.1], "max_depth":[6], "gamma":[0.1]}



                xgb_cv=GridSearchCV(xgb, params, scoring='neg_mean_squared_error', cv=10, n_jobs=-1)
                y = self.df_train[at]
                ...
                self.xgbs[at] = xgb_cv
                with open(f'/cluster/scratch/dominikz/final_missing_{at}.pkl', 'wb+') as f:
    ...
    for index in indices:
        data_full=pd.read_csv("/cluster/scratch/dominikz/raw_data/trainset_full.csv")
        sib = SIB()
